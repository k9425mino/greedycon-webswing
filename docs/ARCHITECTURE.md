...
| 카메라 기울기 / 흔들림 | 최대 14° (앵커 좌우 오프셋 기준) / 속도·지면 근접·부착 충격 |
...
- 흔들림은 속도분과 지면 근접분을 더하고, 부착 순간에만 짧은 충격을 얹는다. 난수 대신 주기가 다른 사인 두 개를 겹쳐 프레임마다 재현된다.
...
지면 근접 보너스(2026-09-16 추가)는 `progress.nearGroundHeightM`(12m) 아래를 `progress.nearGroundMinSpeedMs`(18m/s) 이상으로 지날 때만 쌓인다. 낮을수록 크고(지면에서 만점, 기준 높이에서 0) 초당 `progress.nearGroundBonusPerSecond`(40)를 비율로 나눠 준다. 위험을 감수할 이유를 만들기 위한 것이라 느리게 떠 있는 것에는 주지 않는다. 같은 판정을 화면 가장자리 경고와 HUD 표시가 함께 쓴다.
