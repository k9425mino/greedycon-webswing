...
단일 방문객의 스마트폰 입력을 노트북 게임에 전달하는 웹 애플리케이션이다. 서버는 연결과 입력 중계만 맡고, 게임 상태·물리·점수는 노트북이 결정한다. 현재 센서·터치·연결·정면 보정 프로토타입이 있으며, 아래 구조 중 물리·월드·점수는 미구현이다. 성능과 실기기는 검증하지 않았다. [구현 현황](IMPLEMENTATION.md)을 참고한다.
