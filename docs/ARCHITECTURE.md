...
단일 방문객의 스마트폰 입력을 노트북 게임에 전달하는 웹 애플리케이션이다. 서버는 연결과 입력 중계만 맡고, 게임 상태·물리·점수는 노트북이 결정한다. 현재 센서·터치·연결·정면 보정과 고정 연습 구간의 스윙 물리·추락 종료·재시작이 구현되어 있다. 아래 구조 중 무한 월드·점수·정체 종료는 미구현이다. 성능과 실기기는 검증하지 않았다. [구현 현황](IMPLEMENTATION.md)을 참고한다.
...
로컬 전방 위치가 1,000m를 넘으면 플레이어·건물·앵커·카메라 보간 상태를 같은 양만큼 옮기고 누적 거리 offset에 반영한다. 정체 계산은 offset을 포함한 최고 전진 거리를 사용한다.
