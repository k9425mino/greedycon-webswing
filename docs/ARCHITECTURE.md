...
조준 방향 ray가 사거리 내 건물에 맞으면 그 접촉점을 사용한다. 실패 시 전방 건물 벽면에 미리 배치한 부착 후보 중 보정 원뿔 안의 점을 탐색한다. 플레이어에서 후보까지 raycast해 가시성을 확인한 뒤 조준선과 각도가 가장 작은 점을 선택하고, 동률이면 가까운 점을 선택한다. 후보 마커와 실제 발사 판정은 같은 함수를 사용한다. 어느 쪽에서도 표적을 찾지 못하면 조준 방향으로 거미줄이 최대 사거리까지 뻗었다가 흐려지는 짧은 연출과 실패음을 낸다(눌렀는데 아무 반응이 없어 보이지 않게 한다).

발사선은 카메라 원점에서 시작하면 한 점으로 투영되므로, 시각 효과의 시작점만 카메라 오른쪽 아래로 옮긴다(`effects.fireBeamOriginOffsetM`, 체감 검증 전 제안 초기값). 실제 조준 ray와 표적점은 이동시키지 않는다. 빗나감 효과의 경과 시간은 렌더 프레임 시각을 기준으로 계산한다.
