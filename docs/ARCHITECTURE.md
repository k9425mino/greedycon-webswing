...
- 부착점을 좌표로만 기억하고 Rapier joint는 만들지 않는다. 부착점은 실제로 맞은 지점 그대로 고정되고, 줄의 표시 길이는 플레이어가 움직이는 대로 변한다.
- 부착 중에는 매 step 적분 직전에 보조 속도와 줄 구속을 차례로 더한다. 전진 보조의 기준은 부착점이 아니라 도로 전방(-Z)이고, 호를 만드는 것은 줄 구속이다.
  ...
  - 상하 당김은 고도에 따라 줄인다. `assistCeilingM` 위에서는 0이 되어(그 아래 `assistCeilingFadeM` 구간에서 선형) 계속 붙잡고 있는 사람이 건물 위 하늘로 솟지 않는다.
- 부착점까지 남은 전방 거리가 `assistFadeDistanceM` 이내면 보조를 선형으로 줄인다. 부착점을 지나면 전진 성분만 `assistTailSec` 동안 남기고(앵커 방향 당김은 이제 뒤로 끌기 때문에 끈다) 그 뒤 이 부착의 보조는 끝난다. 뒤로 밀려도 다시 켜지지 않는다.
- **줄 구속(2026-09-16 추가).** 전진 보조만으로는 궤적이 평탄해(측정 진폭 6m) 스윙 리듬이 없었다. 부착 순간의 거리를 줄 길이로 잡고, 내려가는 동안에만 `rope.reelSpeedMps`로 감는다(그네 구르기). 줄보다 멀어지면 바깥 방향 속도 성분을 `rope.tautVelocityRemoval`만큼 잘라 내고, 늘어난 만큼 `rope.restoringAccelPerM`으로 되돌린다. 남는 것은 접선 속도뿐이라 낙하가 호로 바뀐다.
  - 좌우(X)는 구속에서 뺀다. 앵커가 도로 양옆 벽면에 있어 3차원 그대로 당기면 줄이 플레이어를 자기가 붙은 벽으로 끌고 가 벽에 붙어 멈춘다(실제 플레이에서 확인). 상하·전후 평면의 진자로 두고, 좌우는 부착 중 감쇠·중앙 복귀로 다룬다(`rope.lateralDampingPerSec`·`rope.centeringAccelPerM`).
  - 줄이 전진 속도를 `rope.minForwardSpeedMps` 밑으로 깎지 못하게 하고, 상승 속도도 `rope.maxRiseSpeedMps`로 제한한다. 진자는 전진을 높이로 바꾸므로(에너지 보존) 이 두 상한이 없으면 정점에서 전진이 음수가 되거나 고도가 100m 이상으로 솟는다.
- 줄은 손을 떼지 않아도 **앵커 높이까지 솟아오르면** 풀린다. 사거리(`web.maxDistance`)보다 길어져도 끊는다. 뒤로 넘어간 앵커는 잡아당기기만 해 전진이 멈추기 때문이다. 다음 발사는 손을 뗐다 다시 눌러야 나간다(자동 재발사는 없다).
...
| 줄 길이 | 부착 순간의 거리에서 시작, 하강 중 16m/s로 감김, 하한 14m |
| 줄 구속 | 바깥 속도 90% 제거 + 늘어난 길이 × 10m/s²(상한 20), 좌우 제외 |
| 줄이 지키는 한계 | 전진 16m/s 이상 / 상승 16m/s 이하 |
| 부착점 방향 당김 (좌우 / 상하) | 4m/s² / 32m/s² (고도 52m부터 22m 구간에서 0으로) |
| 보조 감소 구간 | 부착점 앞 5m (통과 후 전진 성분만 1.2초) |
| 부착 중 좌우 | 감쇠 3/s + 중앙 복귀 2.5m/s²/m (상한 14m/s²) |
| 카메라 수직 FOV | 90°, 속도 14~42m/s에서 106°까지 |
| 카메라 기울기 / 흔들림 | 최대 14° (앵커 좌우 오프셋 기준) / 속도 0.018m·지면 근접 0.033m·부착 충격 0.07m |

### 표적 선택과 카메라

카메라는 플레이어 위치를 따라가되 회전·롤을 강체에서 물려받지 않는다. **바라보는 방향은 항상 도로 전방(-Z)이다.** 조준은 폰이 하므로 시선까지 속도·스윙에 맞춰 돌리면 조준점이 화면 밖으로 밀려난다. 속도감은 시야각·기울기·흔들림으로만 준다(`client/host/cameraFeel.ts`).

- 시야각은 속도에 따라 넓어지고(`camera.fovSpeedRangeMs` → `camera.maxFovDeg`), 지수 보간으로 수렴한다. 같은 화면 좌표라도 시야각이 바뀌면 조준 방향이 달라지므로, 화면 좌표로 조준하는 검증은 표적 유무를 먼저 확인해야 한다.
- 기울기는 줄이 걸린 쪽(앵커의 좌우 오프셋)을 따른다. 줄 구속이 좌우 속도를 거의 만들지 않으므로 속도가 아니라 앵커 방향을 기준으로 삼는다. 놓으면 천천히 풀린다.
- 흔들림은 속도분과 지면 근접분을 더하고, 부착 순간에만 짧은 충격을 얹는다. 난수 대신 주기가 다른 사인 두 개를 겹쳐 프레임마다 재현된다. 첫 값(0.12/0.22/0.45m)은 화면에서 너무 세서 2026-09-17에 85% 줄였다. 지면 근접을 붉은 화면 가장자리로도 알리던 것은 같은 날 제거했다(화면을 붉게 덮는 것이 거슬린다는 사용자 판단). 지면 근접은 흔들림과 HUD 보너스 표시로만 알린다.
...
정체 기준점은 시작 위치다. 최고 전진 거리가 기준보다 5m 늘면 기준점을 현재 최고값으로 옮기고 타이머를 초기화한다. 5초 경고, 8초 종료다. 한 프레임 안에서 전진 갱신을 먼저 반영하고 종료를 판정한다. 점수는 `floor(생존 시간 × 10 + 지면 근접 보너스)`다.

지면 근접 보너스(2026-09-16 추가)는 `progress.nearGroundHeightM`(12m) 아래를 `progress.nearGroundMinSpeedMs`(18m/s) 이상으로 지날 때만 쌓인다. 낮을수록 크고(지면에서 만점, 기준 높이에서 0) 초당 `progress.nearGroundBonusPerSecond`(40)를 비율로 나눠 준다. 위험을 감수할 이유를 만들기 위한 것이라 느리게 떠 있는 것에는 주지 않는다. HUD에 "아슬아슬 +N"으로 보인다.
